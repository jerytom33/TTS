...
import { Plus, Search, Video, Play, Download, MoreHorizontal, Edit, Trash2, Copy, Clock, CheckCircle, XCircle } from 'lucide-react'
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator, DropdownMenuTrigger } from '@/components/ui/dropdown-menu'
import { Project, GeneratedVideo } from '@/types'
import { useAuth } from '@/contexts/auth-context'
import VideoCreator from '@/features/video-creator/video-creator'
...
export default function ProjectDashboard() {
  ...
  const getVideoStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'PROCESSING':
        return <Badge variant="default"><Play className="w-3 h-3 mr-1" />Processing</Badge>
      case 'COMPLETED':
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">
                    {project.textContent.length} characters
                  </span>
                  <span className="text-muted-foreground">
                    {project.generatedVideos.length} video{project.generatedVideos.length !== 1 ? 's' : ''}
                  </span>
                </div>
